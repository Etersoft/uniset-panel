...
// Keep in sync with OVERVIEW_STATE_VERSION in 58-overview-state.js —
// the constant is not visible across indirect-eval scopes.
const CURRENT_OVERVIEW_STATE_VERSION = 2;

describe('overviewStateDefault', () => {
    it('has required shape', () => {
        const s = overviewStateDefault();
        expect(s.v).toBe(CURRENT_OVERVIEW_STATE_VERSION);
        expect(s.zoom).toBe(1);
        // Wires are hidden by default; minimap is shown.
        expect(s.toggles.wires).toBe(false);
        expect(s.toggles.minimap).toBe(true);
...
describe('load + save round-trip', () => {
    ...
    it('resets on version mismatch', () => {
        ...
        const loaded = loadOverviewState('srv-1');
        expect(loaded.v).toBe(CURRENT_OVERVIEW_STATE_VERSION);

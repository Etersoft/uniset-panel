...
describe('buildVariablesSections', () => {
    it('splits snapshot into inputs/outputs/locals', () => {
        const snap = {
...
describe('renderVariables', () => {
    function makeInst() {
        ...
        inst.state.varsCollapsed = {
            inputs: false, outputs: false, locals: false
        };
        inst.snapshot = {
    ...
    it('renders three sections (even when some are empty)', () => {
        const inst = makeInst();
        ...
        expect(root.querySelector('[data-section="fb_instances"]')).toBeNull();
    });
